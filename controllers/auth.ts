...
export class auth {
    ...
    accountActivation(req: Request, res: Response){
        const { token } = req.body;

        if (token) {
            jwt.verify(token, process.env.JWT_ACCOUNT_ACTIVATION as string, function(
            err: any,
            decoded : any
            ) {
            if (err) {
...
// })
    ...
    adminMiddleware(req: any, res: Response, next: NextFunction){
        User.findById({ _id: req.user._id }).exec((err: any, user: any) => {
            if (err || !user) {
                return res.status(400).json({
    ...
    forgotPassword(req: any, res: Response){
    ...
    resetPassword(req: Request, res: Response){
        ...
        if (resetPasswordLink) {
            jwt.verify(resetPasswordLink, process.env.JWT_RESET_PASSWORD as string, function(
            err: any,
            decoded: any
            ) {
            if (err) {
    ...
    client: any = new OAuth2Client(process.env.GOOGLE_CLIENT_ID);
